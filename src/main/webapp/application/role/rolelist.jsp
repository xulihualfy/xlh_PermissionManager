<%@ page language="java" contentType="text/html; charset=UTF-8"
         pageEncoding="UTF-8"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
<%@taglib prefix="f" uri="http://java.sun.com/jsp/jstl/fmt" %>
<%@taglib prefix="shiro" uri="http://shiro.apache.org/tags" %>

<html>
<head>
    <base href="<%=request.getContextPath() +"/"%>"/>
    <title>角色管理</title>
    <link href="css/H-ui.min.css" rel="stylesheet" type="text/css" />
    <link href="css/H-ui.admin.css" rel="stylesheet" type="text/css" />
    <link href="css/style.css" rel="stylesheet" type="text/css" />
    <link href="lib/Hui-iconfont/1.0.1/iconfont.css" rel="stylesheet" type="text/css" />

<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页
    <span class="c-gray en">&gt;
    </span> 管理员管理 <span class="c-gray en">&gt;
    </span>
    角色管理
    <a class="btn btn-success radius r mr-20"
       style="line-height:1.6em;margin-top:3px"
       href="javascript:location.replace(location.href);"
       title="刷新" >
        <i class="Hui-iconfont">&#xe68f;</i>
    </a></nav>
<div class="pd-20">
    <div class="cl pd-5 bg-1 bk-gray">
        <span class="l">
            <shiro:hasPermission name="role:delete">
            <a href="javascript:;" onclick="batchDel()"
               class="btn btn-danger radius">
                <i class="Hui-iconfont">&#xe6e2;</i>
                批量删除
            </a></shiro:hasPermission>
            <shiro:hasPermission name="role:add">
            <a class="btn btn-primary radius" href="javascript:;"
               onclick="role_add('添加角色','application/role/roleadd.jsp',
               '800')">
                <i class="Hui-iconfont">&#xe600;
                </i> 添加角色
            </a></shiro:hasPermission>
        </span>
        <span class="r">共有数据：<strong>${page.total}</strong> 条</span> </div>
    <table class="table table-border table-bordered table-hover table-bg">
        <thead>
        <tr>
            <th scope="col" colspan="6">角色管理</th>
        </tr>
        <tr class="text-c">
            <th width="25"><input type="checkbox" value="" name="chck_all"></th>
            <th width="40">ID</th>
            <th width="200">角色名称</th>
            <th width="300">描述</th>
            <th width="70">操作</th>
        </tr>
        </thead>
        <tbody>
        <c:forEach items="${page.records}" var="role">
        <tr class="text-c">
            <td><input type="checkbox" value="${role.id}" name="chck_one"></td>
            <td>${role.id}</td>
            <td>${role.roleName}</td>
            <td>${role.roleDesc}</td>
            <td class="f-14">
                <shiro:hasPermission name="role:update">
                <a title="编辑" href="javascript:; "
                   onclick="role_edit('角色编辑','role/getRoleById/${role.id}','1')"
                   style="text-decoration:none">
                <i class="Hui-iconfont">&#xe6df;</i>
                </a></shiro:hasPermission>
                <shiro:hasPermission name="role:delete">
                <a title="删除" href="javascript:;"
                   onclick="role_del(this,${role.id})"
                   class="ml-5" style="text-decoration:none">
                    <i class="Hui-iconfont">&#xe6e2;
                    </i>
                </a></shiro:hasPermission>
            </td>
        </tr>
        </c:forEach>

        </tbody>
    </table>
    <jsp:include page="/common/page.jsp"/>
</div>
<script type="text/javascript" src="lib/jquery/1.9.1/jquery.min.js"></script>
<script type="text/javascript" src="lib/layer/1.9.3/layer.js"></script>
<script type="text/javascript" src="lib/My97DatePicker/WdatePicker.js"></script>
<script type="text/javascript" src="js/H-ui.js"></script>
<script type="text/javascript" src="js/H-ui.admin.js"></script>
<script type="text/javascript" src="js/batch-util.js"></script>
<script type="text/javascript">
    /*管理员-角色-添加*/
    function role_add(title,url,w,h){
        layer_show(title,url,w,h);
    }
    /*管理员-角色-编辑*/
    function role_edit(title,url,id,w,h){
        layer_show(title,url,w,h);
    }
    /*批量删除 */

    function batchDel() {
        var one=$("input[name='chck_one']:checked");
        allBatchDel(one,"role/batchDel")
    }

    /*管理员-角色-删除*/
    function role_del(obj,id){
        var rid=new Object();
        rid.id=id;
        layer.confirm('角色删除须谨慎，确认要删除吗？',function(index){
            //此处请求后台程序，下方是成功后的前台处理……
            $.post("role/delRole",rid,function (result) {
                if (result.state=='SUCCESS') {

                    location.reload();
                    /*$(obj).parents("tr").remove();*/
                    layer.msg('已删除!',{icon:1,time:1000});
                }else {
                    layer.msg(result.msg,{ico: 2,time:1000},function () {
                        var index = parent.layer.getFrameIndex(window.name);
                        parent.$('.btn-refresh').click();
                        parent.layer.close(index);
                    });
                }
            });

        });
    }
</script>
</body>
</html>
